...
+ (CGSize)dc_calculateTextSizeWithText : (NSString *)text WithTextFont: (NSInteger)textFont WithMaxW : (CGFloat)maxW {
    ...
    CGSize textSize = [text boundingRectWithSize:textMaxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:textFont],NSParagraphStyleAttributeName:paragraphStyle} context:nil].size;
...
{
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:[DCSpeedy dc_getCurrentVC].view animated:YES];
...
+ (void)showChessToastWithText:(NSString *)text
{
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:[DCSpeedy dc_getCurrentVC].view animated:NO];
    hud.mode = MBProgressHUDModeText;
    hud.bezelView.style = MBProgressHUDBackgroundStyleSolidColor;
    hud.bezelView.backgroundColor =  [[UIColor blackColor] colorWithAlphaComponent:0.6];
    hud.label.text = text;
